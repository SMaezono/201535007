...
if(win){
  ...
  # Directory: output
  dir_o <- 'C:/Users/Jiachen Liang/OneDrive - OnTheHub - The University of Oxford/Buffa Lab/apobec/output/lm/'
...
exp_import <- function(cancer){
  ...
  input <- as.matrix(read.table(dir_n, sep='\t', header=T, row.names=NULL))
  rn <- input[,1]
  ...
  if(restrict){
    input_n <- input_n[c('APOBEC3A', 'APOBEC3B', 'APOBEC3C', 'APOBEC3D', 'APOBEC3F', 'APOBEC3G', 'APOBEC3H'),]
