...
# Cancer types
names <- c("BLCA", "BRCA", "CESC", "COAD", "HNSC", "KIRC", "KIRP", "LIHC", "LUAD", "LUSC", "PRAD", "STAD", "THCA", "UCEC")
# Omit: OV
...
# Gene names
names.apo <- read.table(file = "apobec/data/tr_a.txt", sep = '\t', header = TRUE)[,2] # UTP11, AK4, HILPDA, MRGBP, CTSV
names.hyp <- read.table(file = "apobec/data/tr_h.txt", sep = '\t', header = TRUE)[,2]

# Median expression values
medians <- read.table(file="apobec/data/2016/median_exp.txt", sep="\t", header=TRUE, row.names=1)
...
fun1 <- function(m, year, state){
  ...
  for(cancer in names){
    ...
    # Import
    dir.apo <- paste("apobec/data/", year, "/", state, "/apobec/", cancer, ".txt", sep="")
    dir.hyp <- paste("apobec/data/", year, "/", state, "/hypoxia/", cancer, ".txt", sep="")
    ...
    thr <- as.numeric(medians[1,cancer])
    ...
    for(apo in rownames(genes.apo)){
      ...
      if(rowSums(!is.na(genes.apo[apo,])) > 3){ # select for rows with more than 3 elements (threshold for correlation test)
        # Test: mean
  ...
  # Write to disk
  write.table(all.cor.mean, paste("apobec/output/", year, "/", state, "/", m, "/mean_cor.txt", sep=""), sep='\t')
  write.table(all.pv.mean, paste("apobec/output/", year, "/", state, "/", m, "/mean_pv.txt", sep=""), sep='\t')
  write.table(all.cor.median, paste("apobec/output/", year, "/", state, "/", m, "/median_cor.txt", sep=""), sep='\t')
  write.table(all.pv.median, paste("apobec/output/", year, "/", state, "/", m, "/median_pv.txt", sep=""), sep='\t')
  write.table(all.cor.pc1, paste("apobec/output/", year, "/", state, "/", m, "/pc1_cor.txt", sep=""), sep='\t')
  write.table(all.pv.pc1, paste("apobec/output/", year, "/", state, "/", m, "/pc1_pv.txt", sep=""), sep='\t')
