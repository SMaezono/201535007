# apobec_cor_allgenes.R

# Jiachen Liang

# Correlate the expression of each APOBEC gene with every gene in the TCGA expression matrix for all cancers
...
dir.create("G:/My Drive/Jiachen Files/Sakura/output/cor_all/per cancer/")
###############################################################################################
# File locations
###############################################################################################
# cancers 8,11 needs to be fixed
if(win) {
...
# List of APOBECs
apobecs <- as.character(read.table(paste(dir_p, 'sign_master.txt', sep = ''),
                                   header=T, sep = '\t')[,1])
...
cor_cal <- function(i) { 
  # Inner function called from hm_cancer()
  
  message(paste(to_time(), '       ', cancers[cancer_index]), apobecs[i],  '- STARTING')
  ...
  for (j in 1:nrow(exp)) { # for all genes
    result = tryCatch({
      if(mean(as.numeric(exp[j,]), na.rm=T)>med) {
  ...
  message(paste(to_time(), '       ', cancers[cancer_index]), apobecs[i],  '- FINISHED')
  
  return(list(cor, pv))
  
}

hm_cancer <- function(cancer_index) { # Main heatmap function
  ...
  message(paste(to_time(), '       ', 'Starting', cancers[cancer_index]))
  ...
  if(file.exists(paste('per cancer/', cancers[cancer_index], '_cor.txt', sep = ''))) { 
    # Read from file system if available
    message(paste(to_time(), '       ', cancers[cancer_index]), '- Reading from file system')
    ...
    message(paste(to_time(), '       ', cancers[cancer_index]), '- Reading done')
  } else { # Compute if not
    
    message(paste(to_time(), '       ', cancers[cancer_index]), '- Calculating correlation')
    ...
    message(paste(to_time(), '       ', cancers[cancer_index]), '- Calculation done')
  }
  ...
  # generate heatmap (top 1000 from hclust)
  # pdf(file=paste('per cancer/', cancers[cancer_index], '_1000_hclust.pdf', sep = ''), width=100, height=10)
  # hm <- heatmap.2(
  #   mat_cor_hc_1000,
  #   Rowv=F, Colv=F, dendrogram = 'none',
  #   col=redgreen,
  ...
  write.table(mat_bin, file=paste('per cancer/', cancers[cancer_index], 
                                  '_bin.txt', sep = ''), sep = '\t')
  write.table(mat_bin[mat_bin$Count>0,], file=paste('per cancer/', 
                                                    cancers[cancer_index],
                                                    '_list.txt', sep = ''),
              sep = '\t')
  write.table(mat_bin[mat_bin$APOBEC3B>0,], file=paste('per cancer/', 
                                                       cancers[cancer_index], 
                                                       '_list_A3B.txt', sep = ''),
              sep = '\t')
  
  message(paste(to_time(), '       ', cancers[cancer_index]), '- FINISHED')
...
if(win) { ### WINDOWS ##########################################################################
  ...
  # Calculate and display correlation (parallel)
  print(Sys.time()-t_last)
  t_last <- Sys.time()
  print('Co-expression analysis')
  cores <- min(detectCores(), length(cancers))
  cl <- makeCluster(cores, outfile='log_2.txt')
  registerDoParallel(cl)
  cor_all <- foreach(n=1:length(cancers)) %dopar% hm_cancer(n)
  stopCluster(cl)
