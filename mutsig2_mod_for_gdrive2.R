...
for(i in 1:length(cancers)){
  ...
  target_file <- paste('G:/My Drive/Jiachen Files/GDAC/', cancers[i], '_t.txt', sep='')
...
for(i in 1:length(cancers)){
  ...
  sp1 <- ggplot(df, aes(x=`APOBEC3A expression`, y=`APOBEC mutation count`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp1 <- sp1 + labs(x = "APOBEC3A expression", y = "APOBEC mutation count (log2)")
  sp1 <- sp1 + stat_cor(method = "spearman", label.x.npc = "middle")
  sp2 <- ggplot(df, aes(x=`APOBEC3B expression`, y=`APOBEC mutation count`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp2 <- sp2 + labs(x = "APOBEC3B expression", y = "APOBEC mutation count (log2)")
  sp2 <- sp2 + stat_cor(method = "spearman", label.x.npc = "middle")
  sp3 <- ggplot(df, aes(x=`Hypoxia signature expression`, y=`APOBEC mutation count`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp3 <- sp3 + labs(x = "Hypoxia signature expression", y = "APOBEC mutation count (log2)")
  sp3 <- sp3 + stat_cor(method = "spearman", label.x.npc = "middle")
  pdf(file=paste(cancers[i], '_A3A-mut_count.pdf', sep=''), width=6, height=6)
  print(sp1)
  dev.off()
  pdf(file=paste(cancers[i], '_A3B-mut_count.pdf', sep=''), width=6, height=6)
  print(sp2)
  dev.off()
  pdf(file=paste(cancers[i], '_hyp-mut_count.pdf', sep=''), width=6, height=6)
  print(sp3)
  dev.off()
}
...
for(i in 1:length(cancers_c)){
  ...
  target_file <- paste('G:/My Drive/Jiachen Files/data/2016/tumour/apobec/', cancers_c[i], '.txt', sep='')
  exp_a_t_r[[i]] <- read.table(file=target_file, sep=' ', header=T, row.names=1)
  target_file <- paste('G:/My Drive/Jiachen Files/data/2016/tumour/hypoxia/', cancers_c[i], '.txt', sep='')
  ...
  target_file <- paste('G:/My Drive/Jiachen Files/GDAC/', cancers_c[i], '_t.txt', sep='')
  input <- read.table(file=target_file, sep='\t', header=T, row.names=NULL)
...
for(i in 1:length(cancers_c)){
  ...
  sp1 <- ggplot(df, aes(x=`APOBEC3A expression`, y=`APOBEC signature enrichment`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp1 <- sp1 + labs(x = "APOBEC3A expression", y = "APOBEC signature enrichment")
  sp1 <- sp1 + stat_cor(method = "spearman", label.x.npc = "middle")
  sp2 <- ggplot(df, aes(x=`APOBEC3B expression`, y=`APOBEC signature enrichment`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp2 <- sp2 + labs(x = "APOBEC3B expression", y = "APOBEC signature enrichment")
  sp2 <- sp2 + stat_cor(method = "spearman", label.x.npc = "middle")
  sp3 <- ggplot(df, aes(x=`Hypoxia signature expression`, y=`APOBEC signature enrichment`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp3 <- sp3 + labs(x = "Hypoxia signature expression", y = "APOBEC signature enrichment")
  sp3 <- sp3 + stat_cor(method = "spearman", label.x.npc = "middle")
  pdf(file=paste(cancers_c[i], '_A3A-enr.pdf', sep=''), width=6, height=6)
  print(sp1)
  dev.off()
  pdf(file=paste(cancers_c[i], '_A3B-enr.pdf', sep=''), width=6, height=6)
  print(sp2)
  dev.off()
  pdf(file=paste(cancers_c[i], '_hyp-enr.pdf', sep=''), width=6, height=6)
  print(sp3)
  dev.off()
}
...
for(c in c(1:(length(plot.matrices)))){
...
for(i in 1:length(cancers)){
  ...
  sp1 <- ggplot(df, aes(x=`APOBEC3A expression`, y=`Max of 2 and 13`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp1 <- sp1 + labs(x = "APOBEC3A expression", y = "Max of 2 and 13 signatures")
  sp1 <- sp1 + stat_cor(method = "spearman", label.x.npc = "middle")
  sp2 <- ggplot(df, aes(x=`APOBEC3B expression`, y=`Max of 2 and 13`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp2 <- sp2 + labs(x = "APOBEC3B expression", y = "Max of 2 and 13 signatures")
  sp2 <- sp2 + stat_cor(method = "spearman", label.x.npc = "middle")
  sp3 <- ggplot(df, aes(x=`Hypoxia signature expression`, y=`Max of 2 and 13`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp3 <- sp3 + labs(x = "Hypoxia signature expression", y = "Max of 2 and 13 signatures")
  sp3 <- sp3 + stat_cor(method = "spearman", label.x.npc = "middle")
  sp4 <- ggplot(df, aes(x=`Max of 3A and 3B exp`, y=`Max of 2 and 13`)) +
    geom_point(alpha=0.6, pch=16) + theme(text = element_text(size=20))
  sp4 <- sp4 + labs(x = "Max of 3A and 3B exp", y = "Max of 2 and 13 signatures")
  sp4 <- sp4 + stat_cor(method = "spearman", label.x.npc = "middle")
  pdf(file=paste(cancers[i], '_A3A-mut_dcs.pdf', sep=''), width=6, height=6)
  print(sp1)
  dev.off()
  pdf(file=paste(cancers[i], '_A3B-mut_dcs.pdf', sep=''), width=6, height=6)
  print(sp2)
  dev.off()
  pdf(file=paste(cancers[i], '_hyp-mut_dcs.pdf', sep=''), width=6, height=6)
  print(sp3)
  dev.off()
  pdf(file=paste(cancers[i], '_max3A3B-mut_dcs.pdf', sep=''), width=6, height=6)
  print(sp4)
  dev.off()
}
...
for(j in c(1:length(cancers))){
  sigpath <- paste('G:/My Drive/Jiachen Files/mutsig/signatures_', cancers[j], '.txt', sep='')
  ...
  if(file.exists(paste('dcs_curated_', cancers[j], '.txt', sep=''))){
    ...
    dcs_counts_pc <- read.table(file=paste('dcs_curated_', cancers[j], '.txt', sep=''), 
...
# for(j in c(1:2, 4:(length(cancers)-3), (length(cancers)-1),
#            length(cancers))){
#   sigpath <- paste('G:/My Drive/Jiachen Files/mutsig/signatures_', cancers[j], '.txt', sep='')
#   dcs_counts_pc <- as.data.frame(matrix(nrow=0, ncol=0))
#   if(file.exists(paste('dcs_curated_', cancers[j], '.txt', sep=''))){
...
plot.value <- c()
plot.sig <- c()
for(c in c(1:length(dcs_counts_all))){
...
# for(c in c(1:2, 4:(length(dcs_counts_all)-3), (length(dcs_counts_all)-1),
#            length(dcs_counts_all))){
#   for(i in 1:nrow(dcs_counts_all[[c]])){
#     for(j in 1:ncol(dcs_counts_all[[c]])){
#       plot.cancer <- c(plot.cancer, cancers[c])
#       plot.value <- c(plot.value, dcs_counts_all[[c]][i,j])
...
# for(i in c(1:2, 4:(length(cancers)-3), (length(cancers)-1),
#            length(cancers))){
#   df <- as.data.frame(matrices[[i]])
#   test <- cor.test(df$`Max of 2 and 13`, df$`APOBEC3A expression`, method='spearman')
#   cor_mat[i,1] <- as.numeric(test$estimate)
...
# NEEDS TO BE TWEAKED FROM HERE ##------ Thu Mar 28 23:08:04 2019 ------##
for(i in c(1:length(cancers))) {
  this.exp <- exp_all[[i]]
  this.exp <- this.exp[,common[[i]]]
  for(j in 1:ctrlcount){
