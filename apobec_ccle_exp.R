...
# Output directory
out_dir <- "/Users/jiachen/OneDrive - OnTheHub - The University of Oxford/Buffa Lab/apobec/output/ccle/"
...
for(i in 1:ncol(raw)){
  ...
  if(length(split)==2){ # correct for some samples having a dot in their name
    type <- split[2]
    name_cut <- split[1]
  }else{
    type <- split[3]
