...
cancers <- c("BLCA", "BRCA", "HNSC", "KIRC", "LUAD", "LUSC", "UCEC")
#cancers <- c("BLCA")
...
for(c in 1:length(cancers)){
  ...
  cna.dir <- paste("/Users/Jiachen/Downloads/AdditionalFile3/TCGA/2014_01_15/", cancer, "/tumour/cna_log2_hgncsymbol.txt.gz", sep="")
  all.cna <- read.table(file = cna.dir, sep = '\t', header = TRUE, row.names = 1)
  
  exp.dir <- paste('/Users/jiachen/OneDrive - OnTheHub - The University of Oxford/Buffa Lab/apobec/data/2016/tumour/apobec/', cancer, '.txt', sep='')
  ...
  write.table(ac_cna_cor, file=paste('output/cna_cor/ac_cna_', cancer, '_cor.txt', sep=''), sep='\t')
  write.table(ac_cna_cor_unf, file=paste('output/cna_cor/ac_cna_', cancer, '_cor_unf.txt', sep=''), sep='\t')
  write.table(ac_cna_pv, file=paste('output/cna_cor/ac_cna_', cancer, '_pv.txt', sep=''), sep='\t')
  
  write.table(ac_exp_cor, file=paste('output/cna_cor/ac_exp_', cancer, '_cor.txt', sep=''), sep='\t')
  write.table(ac_exp_cor_unf, file=paste('output/cna_cor/ac_exp_', cancer, '_cor_unf.txt', sep=''), sep='\t')
  write.table(ac_exp_pv, file=paste('output/cna_cor/ac_exp_', cancer, '_pv.txt', sep=''), sep='\t')
  ...
  pdf(file=paste('output/cna_cor/ac_cna_', cancer, '.pdf', sep=''), width=8, height=8)
  hm <- heatmap.2(
    ac_cna_hm,
    Rowv=NA, Colv=NA,
  ...
  dev.off()
  pdf(file=paste('output/cna_cor/ac_exp_', cancer, '.pdf', sep=''), width=8, height=8)
  hm <- heatmap.2(
    ac_exp_hm,
    Rowv=NA, Colv=NA,
    dendrogram="none", density.info="none", trace="none",
...
write.table(mat_cor, file='output/cna_cor/exp_cna_cor.txt', sep='\t')
write.table(mat_cor_unf, file='output/cna_cor/exp_cna_cor_unf.txt', sep='\t')
write.table(mat_pv, file='output/cna_cor/exp_cna_pv.txt', sep='\t')

mat_hm <- mat_cor
mat_hm[is.na(mat_hm)] <- 0

pdf(file='output/cna_cor/exp_cna.pdf', width=8, height=8)
hm <- heatmap.2(
  mat_hm,
  Rowv=NA, Colv=NA,
  dendrogram="none", density.info="none", trace="none",
