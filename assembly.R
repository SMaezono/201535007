...
root <- "/Users/jiachen/OneDrive - OnTheHub - The University of Oxford/Buffa Lab/apobec/data/"
setwd(root)
...
exp <- read.table('/Users/jiachen/OneDrive - OnTheHub - The University of Oxford/Buffa Lab/apobec/GDAC/BRCA_t.txt', sep='\t', header=TRUE, row.names=NULL)
exp_rownames <- exp[,1]
exp <- exp[,-1]
...
mut_manifest <- as.character(read.table('BRCA_mutations/MANIFEST.txt', sep=' ', header=F, row.names=NULL)[,2])
...
for(s in rownames(mat)){
  ...
  mut_raw <- read.table(file=paste('BRCA_mutations/', sx, '-01.maf.txt', sep=''), sep='\t', fill=T, header=T, row.names = NULL, quote = "")
...
# Write table
write.table(mat, file='assembled_exp_clin.txt', sep='\t')
