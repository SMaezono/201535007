...
# RUN apobec_ver3_mod_for_gdrive.R first
...
cancers <- as.character(read.table(paste(dir_p, 'cancer_list.txt',
                                         sep=''), header=T, sep='\t')[,1])
# Exclude "OV" since it only has tumour samples 
cancers <- cancers[-15]
...
cor <- read.table(paste(root, 'Sakura/output/2016/tumour/spearman/sig_mean_cor.txt',
