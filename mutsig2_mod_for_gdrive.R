...
# modified by Sakura Maezono ##------ Mon Mar 11 22:56:35 2019 ------##
...
cancers_alex <- as.character(read.table(file='cancer_list_alex.txt',
                                        sep='\t', header=F)[,1])
cancers <- as.character(read.table(file='cancer_list.txt',
                                   sep='\t', header=F)[,1])
############################################################# 
### EXCLUDE COAD AND UCEC (too few samples)
mut <- data.frame(nrow=96, ncol=0)
for(c in cancers_alex){
  path <- paste('mutational_catalogs/exomes/', c, '/', c, 
                '_exomes_mutational_catalog_96_subs.txt', sep='')
...
for(i in 1:length(cancers)){
  ...
  target_file <- paste('G:/My Drive/Jiachen Files/data/2016/tumour/apobec/',
                       cancers[i], '.txt', sep='')
    exp_a_t[[i]] <- read.table(file=target_file, sep=' ',
                               header=T, row.names=1)
  
  
  target_file <- paste('G:/My Drive/Jiachen Files/data/2016/tumour/hypoxia/', 
                       cancers[i], '.txt', sep='')
    exp_h_t[[i]] <- read.table(file=target_file, sep=' ',
                               header=T, row.names=1)

  target_file <- paste('G:/My Drive/Jiachen Files/GDAC/',
                       cancers[i], '_t.txt', sep='')
...
root <- "G:/My Drive/Jiachen Files/mutsig/output_counts/"
setwd(root)
...
for(i in 1:length(cancers)){
  common[[i]] <- intersect(rownames(mut_sums_apo), 
...
for(i in 1:length(cancers)){
  df <- as.data.frame(matrices[[i]])
  test <- cor.test(df$`APOBEC mutation count`, df$`APOBEC3A expression`,
                   method='spearman')
  cor_mat[i,1] <- as.numeric(test$estimate)
  cor_mat[i,2] <- as.numeric(test$p.value)
  test <- cor.test(df$`APOBEC mutation count`, df$`APOBEC3B expression`, 
                   method='spearman')
  cor_mat[i,3] <- as.numeric(test$estimate)
  cor_mat[i,4] <- as.numeric(test$p.value)
  test <- cor.test(df$`APOBEC mutation count`, df$`Hypoxia signature expression`, 
                   method='spearman')
...
for(i in 1:length(cancers)){
  ...
  for(j in 1:ctrlcount){
    ...
    this.test <- cor.test(as.numeric(this.exp[this.gene,]),
                          as.numeric(mut_sums_apo_log[common[[i]],1]))
...
df_counts <- data.frame(control.gene, control.cancer, control.cor, control.pv,
                        control.cor.f, control.method)

for(i in 1:length(cancers)){
  actual <- as.data.frame(t(c('Hypoxia', cancers[i], cor_mat[i,5], cor_mat[i,6],
                              cor_mat[i,5], 'Counts')))
  colnames(actual) <- colnames(df_counts)
  a3a <- as.data.frame(t(c('A3A', cancers[i], cor_mat[i,1], cor_mat[i,2], 
                           cor_mat[i,1], 'Counts')))
  colnames(a3a) <- colnames(df_counts)
  a3b <- as.data.frame(t(c('A3B', cancers[i], cor_mat[i,3], cor_mat[i,4], 
                           cor_mat[i,3], 'Counts')))
...
for(i in 1:length(cancers_c)) {
  ...
  exp_a_t_r[[i]] <- read.table(file=target_file, sep=' ',
                                 header=T, row.names=1)

  target_file <- paste('G:/My Drive/Jiachen Files/data/2016/tumour/hypoxia/',
                       cancers_c[i], '.txt', sep='')
  exp_h_t_r[[i]] <- read.table(file=target_file, sep=' ',
                                 header=T, row.names=1)
  # Fix names
  ...
  target_file <- paste('G:/My Drive/Jiachen Files/GDAC/', cancers_c[i],
                       '_t.txt', sep = '')
  input <- read.table(file = target_file, sep = '\t', header = T, row.names = NULL)
...
for(i in 1:length(cancers_c)){
  common[[i]] <- intersect(rownames(roberts),
                           intersect(colnames(exp_a_t_r[[i]]), 
                                                        colnames(exp_h_t_r[[i]])))
}
...
for(i in 1:length(cancers_c)){
  df <- as.data.frame(matrices[[i]])
  test <- cor.test(df$`APOBEC signature enrichment`, df$`APOBEC3A expression`,
                   method='spearman')
  cor_mat[i,1] <- as.numeric(test$estimate)
  cor_mat[i,2] <- as.numeric(test$p.value)
  test <- cor.test(df$`APOBEC signature enrichment`, df$`APOBEC3B expression`,
                   method='spearman')
  cor_mat[i,3] <- as.numeric(test$estimate)
  cor_mat[i,4] <- as.numeric(test$p.value)
  test <- cor.test(df$`APOBEC signature enrichment`, df$`Hypoxia signature expression`,
                   method='spearman')
  cor_mat[i,5] <- as.numeric(test$estimate)
  cor_mat[i,6] <- as.numeric(test$p.value)
}
...
for(i in 1:length(cancers_c)){
  actual <- as.data.frame(t(c('Hypoxia', cancers_c[i], cor_mat[i,5],
                              cor_mat[i,6], cor_mat[i,5], 'Enrichment')))
  colnames(actual) <- colnames(df_enrichment)
  a3a <- as.data.frame(t(c('A3A', cancers_c[i], cor_mat[i,1], 
                           cor_mat[i,2], cor_mat[i,1], 'Enrichment')))
  colnames(a3a) <- colnames(df_enrichment)
  a3b <- as.data.frame(t(c('A3B', cancers_c[i], cor_mat[i,3], 
                           cor_mat[i,4], cor_mat[i,3], 'Enrichment')))
...
if(file.exists('dcs_all.txt')){
  ...
  sigpath <- "G:/My Drive/Jiachen Files/mutsig/signatures alt.txt"
...
common <- vector('list', length=length(cancers))
for(i in 1:length(cancers)){
  common[[i]] <- intersect(rownames(dcs_apo), 
                           intersect(colnames(exp_a_t[[i]]),
                                     colnames(exp_h_t[[i]])))
}

root <- "G:/My Drive/Jiachen Files/mutsig/output_dcs/"
setwd(root)
...
for(c in c(1:(length(cancers)-3), (length(cancers)-1),
    length(cancers))) {
...
for(c in c(1:(length(plot.matrices)-3), (length(plot.matrices)-1),
           length(plot.matrices))){
...
for(c in c(1:(length(cancers)-3), (length(cancers)-1),
           length(cancers))){
  df <- as.data.frame(matrices[[i]])
  test <- cor.test(df$`Max of 2 and 13`, df$`APOBEC3A expression`,
                   method='spearman')
  cor_mat[i,1] <- as.numeric(test$estimate)
  cor_mat[i,2] <- as.numeric(test$p.value)
  test <- cor.test(df$`Max of 2 and 13`, df$`APOBEC3B expression`,
                   method='spearman')
  cor_mat[i,3] <- as.numeric(test$estimate)
  cor_mat[i,4] <- as.numeric(test$p.value)
  test <- cor.test(df$`Max of 2 and 13`, df$`Hypoxia signature expression`, 
                   method='spearman')
  cor_mat[i,5] <- as.numeric(test$estimate)
  cor_mat[i,6] <- as.numeric(test$p.value)
  test <- cor.test(df$`Max of 2 and 13`, df$`Max of 3A and 3B exp`,
                   method='spearman')
...
for(i in c(1:(length(cancers)-3), (length(cancers)-1),
           length(cancers))){
  this.exp <- exp_all[[i]]
  this.exp <- this.exp[,common[[i]]]
  for(j in 1:ctrlcount){
...
df_dcs_all <- data.frame(control.gene, control.cancer, control.cor,
                         control.pv, control.cor.f, control.method)

for(i in 1:length(cancers)){
  actual <- as.data.frame(t(c('Hypoxia', cancers[i], cor_mat[i,5], 
                              cor_mat[i,6], cor_mat[i,5], 'DCS all')))
...
for(j in 1:length(cancers)){
  mut_counts_pc[[j]] <- mut_counts[intersect(rownames(mut_counts),
                                             intersect(colnames(exp_a_t[[j]]),
                                                       colnames(exp_h_t[[j]]))),]
}
# CONTINUE FROM HERE ##------ Thu Mar 14 15:46:29 2019 ------##
for(j in 1:length(cancers)){
  sigpath <- paste('G:/My Drive/Jiachen Files/mutsig/signatures_', cancers[j], 
                   '.txt', sep='')
  ...
  if(file.exists(paste('dcs_curated_', cancers[j], '.txt', sep=''))){
    ...
    dcs_counts_pc <- read.table(file=paste('dcs_curated_', cancers[j], 
                                           '.txt', sep=''), header=T, 
                                row.names=1, sep='\t')
    ...
    # Output matrix
    write.table(dcs_counts_pc, file=paste('dcs_curated_', cancers[j], 
                                          '.txt', sep=''), sep='\t')
  }
}
print('Read complete')
...
plot.df <- data.frame(plot.cancer, plot.value, plot.sig)
for(c in 1:length(dcs_counts_all)){
  plot_out <- ggplot(plot.df[plot.df$plot.cancer==cancers[c],],
                     aes(x=plot.sig, y=plot.value)) +
    geom_jitter(alpha=0.1) +
    ...
    theme(text = element_text(size=16),
          axis.text.x = element_text(angle=45, hjust=1))
  pdf(file=paste('output_dcs_curated/', cancers[c], '_DCS_cur.pdf', sep=''),
      width=6, height=4)
  print(plot_out)
  dev.off()
  ...
  write.table(nnp, file=paste('output_dcs_curated/', cancers[c], '_nnp.txt',
                              sep=''), sep='\t')
}
...
for(i in 1:length(cancers)){
  common[[i]] <- intersect(rownames(dcs_apo), 
                           intersect(colnames(exp_a_t[[i]]), colnames(exp_h_t[[i]])))
}

root <- "G:/My Drive/Jiachen Files/mutsig/output_dcs_curated/"
setwd(root)
...
df_dcs_cur <- data.frame(control.gene, control.cancer, control.cor, 
                         control.pv, control.cor.f, control.method)

for(i in 1:length(cancers)){
  actual <- as.data.frame(t(c('Hypoxia', cancers[i], cor_mat[i,5], 
                              cor_mat[i,6], cor_mat[i,5], 'DCS curated')))
  colnames(actual) <- colnames(df_dcs_cur)
  a3a <- as.data.frame(t(c('A3A', cancers[i], cor_mat[i,1], 
                           cor_mat[i,2], cor_mat[i,1], 'DCS curated')))
  colnames(a3a) <- colnames(df_dcs_cur)
  a3b <- as.data.frame(t(c('A3B', cancers[i], cor_mat[i,3], 
                           cor_mat[i,4], cor_mat[i,3], 'DCS curated')))
...
root <- "/G:/My Drive/Jiachen Files/mutsig/output_controls/"
setwd(root)
...
for(i in 1:length(cancers_c)){
  plot_control <- ggplot() +
    geom_boxplot(data=df_all[(df_all$control.cancer==cancers_c[i] & df_all$control.gene!='Hypoxia'),], 
                 aes(x=control.method, y=control.cor)) +
    geom_point(data=df_all[(df_all$control.cancer==cancers_c[i] & df_all$control.gene=='Hypoxia'),],
               aes(x=control.method, y=control.cor), color='red') +
    ylim(-0.5,0.5) +
    ggtitle(paste('Control plot -', cancers_c[i])) +
    xlab("Method") + ylab("Spearman's rho") +
    theme(text = element_text(size=16),
          axis.text.x = element_text(angle=45, hjust=1))
  pdf(file=paste('grouped by cancer/', cancers_c[i], '.pdf', sep=''), 
      width=4, height=4)
  print(plot_control)
  dev.off()
}
