...
# Jiachen Liang
# modified by Sakura ##------ Thu Feb 28 10:31:33 2019 ------##
...
# Parameters
# Set to TRUE to export using gene names; FALSE to export using gene IDs
gene_names <- TRUE
# Export a file with all samples
write_all <- TRUE 
# Export a file with only solid tumour samples
write_t <- TRUE 
# Export a file with only normal samples
if (cancers == "OV") {
  write_n <- FALSE
} else {
  write_n <- TRUE 
...
main <- function(import_index) {
  ...
  if (write_all) {
    ...
    if (gene_names) {
      write.table(out.name, paste(root, cancer, '_all.txt', 
                                  sep=''), sep='\t')
    } else {
      write.table(out.id, paste(root, cancer, '_all.txt', 
                                sep=''), sep='\t')
    }
  }
  ...
  if (write_t) {
    ...
    write.table(out.t, paste(root, cancer, '_t.txt', 
                             sep=''), sep='\t')
  }
  if (write_n) {
    print('   writing normal')
    write.table(out.n, paste(root, cancer, '_n.txt',
                             sep=''), sep='\t')
  }
  if (cancers == "OV") {
    if (write_stats) {
      ...
      # Write
      write.table(stats_all, paste(root, 'stats/', cancer, 
                                   '_all.txt', sep=''), sep='\t')
      write.table(stats_s_a, paste(root, 'stats/', cancer, 
                                   '_s_a.txt', sep=''), sep='\t')
      write.table(stats_s_t, paste(root, 'stats/', cancer, 
                                   '_s_t.txt', sep=''), sep='\t')
      write.table(stats_g_a, paste(root, 'stats/', cancer, 
                                   '_g_a.txt', sep=''), sep='\t')
      write.table(stats_g_t, paste(root, 'stats/', cancer, 
                                   '_g_t.txt', sep=''), sep='\t')
      write.table(de, paste(root, 'stats/', cancer, '_de.txt',
                            sep=''), sep='\t')
      write.table(count_a, paste(root, 'stats/', cancer, 
                                 '_count_a.txt', sep=''), sep='\t')
      write.table(count_t, paste(root, 'stats/', cancer, 
                                 '_count_t.txt', sep=''), sep='\t')
    }
    write_n <- FALSE
  } else {
    if (write_stats) {
    ...
    write.table(stats_s_t, paste(root, 'stats/', cancer, 
                                 '_s_t.txt', sep=''), sep='\t')
    write.table(stats_s_n, paste(root, 'stats/', cancer, 
                                 '_s_n.txt', sep=''), sep='\t')
    write.table(stats_g_a, paste(root, 'stats/', cancer, 
                                 '_g_a.txt', sep=''), sep='\t')
    write.table(stats_g_t, paste(root, 'stats/', cancer, 
                                 '_g_t.txt', sep=''), sep='\t')
    write.table(stats_g_n, paste(root, 'stats/', cancer, 
                                 '_g_n.txt', sep=''), sep='\t')
    write.table(de, paste(root, 'stats/', cancer, '_de.txt',
                          sep=''), sep='\t')
    write.table(count_a, paste(root, 'stats/', cancer, 
    ...
    write.table(count_t, paste(root, 'stats/', cancer, 
                               '_count_t.txt', sep=''), sep='\t')
    write.table(count_n, paste(root, 'stats/', cancer, 
                               '_count_n.txt', sep=''), sep='\t')
    }
  }
  return(cancer)
}
