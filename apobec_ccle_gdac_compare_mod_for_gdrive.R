...
# Correlate CCLE and TCGA data for APOBEC expression (PRAD, BRCA, and COAD)

t_start <- Sys.time()

# Root directory
root <- "G:/My Drive/Jiachen Files/Sakura/output/ccle/"
setwd(root)

# TCGA directory
tcga_dir <- "G:/My Drive/Jiachen Files/data/stats/"
...
plot <- ggplot(prad_mean_bl, aes(x=CCLE, y=TCGA)) +
  geom_point()
pdf(file="prad_mean_bl.pdf", width=15, height=9)
print(plot)
dev.off()
...
plot <- ggplot(brca_mean_bl, aes(x=CCLE, y=TCGA)) +
  geom_point() +
  geom_text(aes(label=rownames(brca_mean_bl)),hjust=0, vjust=0, size=2)
pdf(file="brca_mean_bl.pdf", width=15, height=9)
print(plot)
dev.off()
...
plot <- ggplot(coad_mean_bl, aes(x=CCLE, y=TCGA)) +
  geom_point() +
  geom_text(aes(label=rownames(brca_mean_bl)),hjust=0, vjust=0, size=2)
pdf(file="coad_mean_bl.pdf", width=15, height=9)
print(plot)
dev.off()
...
plot <- ggplot(prad_mean_fc, aes(x=CCLE, y=TCGA)) +
  geom_point() +
  geom_text(aes(label=rownames(brca_mean_bl)),hjust=0, vjust=0, size=2)
pdf(file="prad_mean_fc.pdf", width=15, height=9)
print(plot)
dev.off()
...
plot <- ggplot(brca_mean_fc, aes(x=CCLE, y=TCGA)) +
  geom_point() +
  geom_text(aes(label=rownames(brca_mean_bl)),hjust=0, vjust=0, size=2)
pdf(file="brca_mean_fc.pdf", width=15, height=9)
print(plot)
dev.off()
...
plot <- ggplot(coad_mean_fc, aes(x=CCLE, y=TCGA)) +
  geom_point() +
  geom_text(aes(label=rownames(brca_mean_bl)),hjust=0, vjust=0, size=2)
pdf(file="coad_mean_fc.pdf", width=15, height=9)
print(plot)
dev.off()
