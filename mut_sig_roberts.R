...
root <- "C:/Users/Jiachen Liang/OneDrive - OnTheHub - The University of Oxford/Buffa Lab/apobec/mutsig/"
setwd(root)
...
cancers <- as.character(read.table(file='cancers_list_roberts.txt', sep='\t', header=F)[,1])
...
root <- "C:/Users/Jiachen Liang/OneDrive - OnTheHub - The University of Oxford/Buffa Lab/apobec/mutsig/output_enrichment/"
setwd(root)

# Get common samples
common <- vector('list', length=length(cancers))
for(i in 1:length(cancers)){
  common[[i]] <- intersect(rownames(roberts), intersect(colnames(exp_a_t[[i]]), colnames(exp_h_t[[i]])))
}
