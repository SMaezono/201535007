  ...
  # Root directory
  dir.create("G:/My Drive/Jiachen Files/Sakura/output/cor_all/enrichment/")
  ...
  ###############################################################################
  dir.create("G:/My Drive/Jiachen Files/Sakura/output/cor_all/enrichment/plots")
  # Plot functions
  ...
  plot_out <- function(m, cancer, group){
    ...
    out <- GOChord(m, nlfc=0, gene.size=gs, gene.space=gp,
                  gene.space=gp, lfc.col=c('cyan','black','red'))
  ...
  plot_exp <- function(m, cancer, group){
    if(nrow(m)>70){gs<-2.5;gp<-0.145}else{gs<-3.5;gp<-0.17}
    out <- GOChord(m, gene.order = 'logFC', gene.size=gs, 
                   gene.space=gp, lfc.col=c('cyan','black','red'))
