...
dest <- 'G:/My Drive/Jiachen Files/data/stats/'
...
# Genes
genes_dir <- 'G:/My Drive/Jiachen Files/data/'
...
stats <- function(import_index){
  ...
  if (cancer == "OV") {
    # Import
    stats_a <- read.table(file=paste(root, 'stats/', cancer, '_g_a.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
    stats_t <- read.table(file=paste(root, 'stats/', cancer, '_g_t.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
    stats_de <- read.table(file=paste(root, 'stats/', cancer, '_de.txt', sep = ''),
                           sep = '\t', header=TRUE, row.names=NULL)
    count_a <- read.table(file=paste(root, 'stats/', cancer, '_count_a.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
    count_t <- read.table(file=paste(root, 'stats/', cancer, '_count_t.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
    # Fix names
    names <- stats_a[,1]
    ...
    stats_t <- read.table(file=paste(root, 'stats/', cancer, '_g_t.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
    stats_n <- read.table(file=paste(root, 'stats/', cancer, '_g_n.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
    stats_de <- read.table(file=paste(root, 'stats/', cancer, '_de.txt', sep = ''),
                           sep = '\t', header=TRUE, row.names=NULL)
    count_a <- read.table(file=paste(root, 'stats/', cancer, '_count_a.txt', sep = ''),
    ...
    count_t <- read.table(file=paste(root, 'stats/', cancer, '_count_t.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
    count_n <- read.table(file=paste(root, 'stats/', cancer, '_count_n.txt', sep = ''),
                          sep = '\t', header=TRUE, row.names=NULL)
